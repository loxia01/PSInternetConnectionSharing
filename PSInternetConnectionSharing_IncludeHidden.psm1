...
#>
    ...
    param(
        [Parameter(Mandatory)]
        [ValidateScript({
            if (((Get-NetAdapter -Name $_ -IncludeHidden).Name -eq $_) -and ((Get-NetAdapter -Name $_ -IncludeHidden).Status -notin 'Not Present','Disabled')) { $true }
    ...
    process
    {
        ...
        {
            $publicConnectionName = (Get-NetAdapter $PublicConnectionName -IncludeHidden).Name
            $privateConnectionName = (Get-NetAdapter $PrivateConnectionName -IncludeHidden).Name
            Write-Host "`nICS is already set for network connections $publicConnectionName (public) and $privateConnectionName (private).`n"
        }
        else
        { 
            $netAdapters = Get-NetAdapter -IncludeHidden
...
#>
    ...
    param(
        ...
        [ValidateScript({
            foreach ($ConnectionName in $_) {
                if ((Get-NetAdapter -Name $_ -IncludeHidden).Name -eq $_) { $true }
    ...
    process
    {
        if ($ConnectionNames)
        {
            ...
            {
                $connectionName = (Get-NetAdapter -Name $ConnectionName -IncludeHidden).Name
