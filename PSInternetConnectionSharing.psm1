...
#>
    ...
    process
    {
        ...
        {   
            ...
            {
                $output += [pscustomobject]@{NetworkConnectionName=$connectionName + "    "; StatusICS='Disabled'; ConnectionType=''}
            }
            if ($connectionConfig.SharingEnabled -eq $true -and $connectionConfig.SharingConnectionType -eq 0)
            {
                $output += [pscustomobject]@{NetworkConnectionName=$connectionName + "    "; StatusICS='Enabled'; ConnectionType='Public'}
            }
            if ($connectionConfig.SharingEnabled -eq $true -and $connectionConfig.SharingConnectionType -eq 1)
            {
                $output += [pscustomobject]@{NetworkConnectionName=$connectionName + "    "; StatusICS='Enabled'; ConnectionType='Private'}
            }
        }
    }
    end
    {
        if ($ShowEnabled)
        {
            $output | Sort-Object ConnectionType -Descending | Where-Object StatusICS -EQ Enabled
        }
        elseif ($output.ConnectionType -ne '')
        {
