...
#>
    ...
    param(
        [Parameter(Mandatory)]
        [ValidateScript({
            if (((Get-NetAdapter -Name $_).Name -eq $_) -and ((Get-NetAdapter -Name $_).Status -ne "Not Present") -and
                ((Get-NetAdapter -Name $_).Status -ne "Disabled")) { $true }
    ...
    begin
    {
        ...
        {
            Write-Error -Message "This function needs administrator rights." -Category PermissionDenied
            break
        }
    ...
    process
    {
            ...
            ($privateConnectionConfig.SharingEnabled -eq "True" -and $privateConnectionConfig.SharingConnectionType -eq 1))
        {
            ...
            Write-Host `n"ICS is already set for network connections $publicConnectionName (public) and $privateConnectionName (private)."`n
        }
        else
        { 
            ...
            foreach ($connectionName in $netAdapters.Name)
            {
                if ($connectionName -eq $PublicConnectionName)
                {
                    ...
                    if ($? -eq "True")
                    {
                        Write-Host `n"ICS was enabled for network connection $connectionName (public connection)."
                    }
                }
    ...
    process
    {
        if (!$ConnectionNames)
        {
            ...
            foreach ($connectionName in $netAdapters.Name)
            {
                ...
                {
                    $output.Add($connectionName,"Disabled")
                }
                if ($connectionConfig.SharingEnabled -eq "True" -and $connectionConfig.SharingConnectionType -eq 0)
                {
                    $output.Add($connectionName,"Enabled (public)")
                }
                if ($connectionConfig.SharingEnabled -eq "True" -and $connectionConfig.SharingConnectionType -eq 1)
                {
                    $output.Add($connectionName,"Enabled (private)")
                }
            }
            $output.GetEnumerator() | Select-Object @{Label="NetworkConnectionName";Expression={$_.Key}}, @{Label="StatusICS";Expression={$_.Value}}
        }
        else
        {
            $output = @{}
    ...
    process
    {
        ...
        foreach ($connectionName in $netAdapters.Name)
        {
            ...
            if ($connectionConfig.SharingEnabled -eq "True")
            {
                ...
                {
                    Write-Host `n"ICS was disabled for network connection $connectionName."
                }
        ...
        if ($icsDisabled -eq 0)
        {
            Write-Host `n"ICS was already disabled for all network connections."
        }
        Write-Host ""
    }
}
