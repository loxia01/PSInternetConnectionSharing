$TestAdmin = {
    if (-not ([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::
        ...
        throw "This function requires administrator rights. Restart PowerShell using the Run as administrator option."
    }
}
...
.NOTES
 ...
 Testing for elevated permissions is done initially in function.
...
#>
    ...
    begin
    {
        if (-not $ConnectionNames) { & $TestAdmin }
        regsvr32.exe -s hnetcfg.dll
        $netShare = New-Object -ComObject HNetCfg.HNetShare
    }
    ...
    process
    {
        if ($ConnectionNames)
        {
            ...
            $output | Format-Table -Property @{Name='NetworkConnectionName'; Expression={$_.Key}}, @{Name='StatusICS'; Expression={$_.Value}}
        }   
        else
        {
            $output = @{}
